  ...
  methods: {
    async fetchData(uuid) {
      try {
        ...
        const foundMunicipality = data.find(
          (municipality) => municipality.uuid === uuid
        );
    ...
    <div class="row g-3 my-3 px-2 px-md-0">
      <div class="col-lg-7">
        ...
        <div class="municipal-info pt-5">
          ...
          <div
            v-if="municipalityDetails && municipalityDetails.history"
            class="pb-2"
          >
          ...
          <div
            v-if="municipalityDetails && municipalityDetails.officials"
            class="pb-2"
          >
            <div class="ck-content py-4">
              <MunicipalOfficialsTemplateComponentVue
                v-if="isOfficialsObject"
                :municipalityOfficials="municipalityDetails.officials"
              ></MunicipalOfficialsTemplateComponentVue>
            </div>
          </div>
          ...
          <!-- Checks if the municipal officials is empty, if it is then it will not render -->
          <div
            v-if="municipalityDetails && municipalityDetails.information"
            class="pb-2"
          >
            <div
              class="ck-content py-2"
              v-if="
                Array.isArray(municipalityDetails.information) &&
                municipalityDetails.information.length > 0
              "
            >
              <MunicipalityInformationTableComponent
                :municipalityDetails="municipalityDetails.information"
              ></MunicipalityInformationTableComponent>
            </div>
          </div>
          ...
          <div
            v-if="
              municipalityDetails &&
              municipalityDetails.activities &&
              municipalityDetails.activities.rows &&
              municipalityDetails.activities.rows.length > 0
            "
            class="pb-2"
          >
            <div class="ck-content py-4">
              <MunicipalityActivitiesTableComponentVue
                :municipalityDetails="municipalityDetails.activities"
              ></MunicipalityActivitiesTableComponentVue>
            </div>
          </div>
        </div>
      ...
      <div class="col-lg-5">
        <SidebarListComponent :listType="'municipalityList'">
          <template #heading>Municipalities of Quezon Province</template>
        </SidebarListComponent>
        <SidebarListComponent :listType="'hotlineList'">
